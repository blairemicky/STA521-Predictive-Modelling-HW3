...
Next, we reproduce the two plots in Figure 5.1 in Gelman and Hill. For the secon plot, we consulted their code to understand what they did: simulate 20 model coefficients based on the sampling distributions and create the corresponding plots. We repeated this procedure and used ggplot2 to make the corresponding plots.
...
ANSWER: First we compute the residual deviance, defined as the change in deviance between the fitted model and the saturated model. 

```{r}
vote.glm0 <-glm(vote ~ 1 ,data=nes1992, family = binomial(link = "logit"))
vote.glm1$deviance
pchisq(vote.glm1$deviance, vote.glm1$df.residual, lower.tail = F)
anova(vote.glm0, vote.glm1, test="Chi")
anova(vote.glm1, test="Chi")
par(mfrow=c(2,2)) 
...
=======
OR<-data.frame(est=numeric(length(yr)), CI.lwr=numeric(length(yr)),CI.upr=numeric(length(yr)) )
>>>>>>> cb6a8ad5a35c9d18a70a944f547dbe06617185ca
>>>>>>> 2f707592a9e256813179e2378e0d41ba903de503
...
levels(nes1992_clean$partyid3) = list("democraets" = 1, "independents" = 2, "republicans" = 3, "apolitical" = 9)
##political ideology
...
```{r}
##delete row corresponding to apolitical
nes1992_clean = nes1992_clean[-126,]

glm_1992clean = glm(vote~  income + gender + race + educ1 + partyid3 + ideo + race:gender + gender:partyid3, 
                           data = nes1992_clean, 
                           family = binomial(link = "logit"),
                           maxit = 100)

summary(glm_1992clean)
```
...
```{r}
glm_1992clean_al1 = glm(vote~  income + gender + race + educ1 + partyid3 + race:gender + gender:partyid3, 
                           data = nes1992_clean, 
                           family = binomial(link = "logit"),
                           maxit = 100)

summary(glm_1992clean_al1)

anova(glm_1992clean_al1,glm_1992clean)
```
...
pred = predict(glm_1992clean,type = "response")
