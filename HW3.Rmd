...
```{r}
curve (invlogit(rep$coef[1] + rep$coef[2]*x), 1, 5, ylim=c(-.01,1.01),
         xlim=c(-2,8), xaxt="n", xaxs="i", mgp=c(2,.5,0),
         ylab="Pr (Republican vote)", xlab="Income", lwd=4, col="firebrick")
  curve (invlogit(rep$coef[1] + rep$coef[2]*x), -2, 8, lwd=.5, add=T)
  axis (1, 1:5, mgp=c(2,.5,0))
  mtext ("(poor)", 1, 1.5, at=1, adj=.5)
  mtext ("(rich)", 1, 1.5, at=5, adj=.5)
  points (jitter (income, .5), jitter (as.numeric(vote), .08), pch=20, cex=.05, col=alpha("pink", .25))
```
...
glm_1992clean = glm(vote~  gender + race + educ1 + partyid3 + ideo + race:gender + gender:partyid3, 
                           data = nes1992_clean, 
