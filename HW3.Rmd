...
a = binned.resids(ideo_feel_flat, res)
...
```{r}
##add interaction
glm_1992_inr_na = glm(vote~  income + gender + race + educ1 + partyid3 + ideo + race:gender + gender:partyid3, 
                           data = nes1992_na, 
                           family = binomial(link = "logit"),
                           maxit = 100)

ci_na = exp(cbind(coef(glm_1992_inr_na),confint(glm_1992_inr_na)))
suppressMessages(print(ci_na))
colnames(ci_na) = c("RR", "2.5", "97.5")
print(xtable(round(ci_na, 4)), comment=F)
```
