...
ideo_feel_flat = as.numeric(data.matrix(nes1992_clean["ideo_feel"]))
binnedplot(ideo_feel_flat, res, xlab = "ideo_feel")
...
```{r}
glm_1992_rmideo = glm(vote~  income + gender + race + educ1 + partyid3 + race:gender + gender:partyid3, 
                           data = nes1992_clean, 
                           family = binomial(link = "logit"),
                           maxit = 100)
...
summary(glm_1992_rmideo)

anova(glm_1992_inr, glm_1992_idf,test = "Chisq")
```
...
pred = predict(glm_1992_idf,type = "response")
