...
for (i in 1:length(yr)) {
if (i==1) {
  inc <- rep_interact$coefficients[2]
} else {
inc <- rep_interact$coefficients[2] + rep_interact$coefficients[i + 13]
}
inc.est <- rbind(inc.est, inc, deparse.level = 0)
}
inc.est
```
...
```{r}
plot(x = rep(inc.est, 2), y = odds.ci, pch=20, col="darkblue")
for (i in 1:length(yr)){
  lines(rep(inc.est[i],2), c(odds.ci[i,1], odds.ci[i,2]), lwd=.5, col="royalblue")
}
```
