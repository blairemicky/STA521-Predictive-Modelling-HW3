...
```{r}
# add code in this chunk and click green arrow on right to run or use the Run menu.   Add additional chunks using the Insert menu or short-cut keys.  
```
...
```{r}
rep_ci = c(1, length(unique(nes52.2000$year)))
for(i in 1:length(unique(nes52.2000$year))) {
rep_ci[i] <- glm(rep_presvote ~ income, data = nes52.2000, family=binomial(link="logit"), subset=year==nes52.2000$year[i])
}
```
...
```{r}
rep_interact <- glm(rep_presvote~income*factor(year), data = nes52.2000, family=binomial(link="logit"))
rep_int_ci<- confint(rep_interact)
rep_interact
```
...
```{r}
ggplot(year, rep_interact, aes(year, rep_interact))
curve(rep_interact)
```
