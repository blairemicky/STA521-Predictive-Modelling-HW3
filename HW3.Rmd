...
```{r}
rep.52 <- glm(vote ~ factor(income), data = nes52.2000, family=binomial(link="logit"), subset= year==1952)
rep.52
odds.rich <- exp(coef(rep.52)[1] + coef(rep.52)[5]*mean(rep.52$coefficients[5]))
odds.poor <- exp(coef(rep.52)[1])
odds.ratio <- odds.rich/odds.poor
odds.ratio
```
...
```{r}
plot(data = nes52.2000, mapping = aes(unique(year==1952), odds.ratio))
```
